...
contract BBTSquare {
    ...
    event rewardRequested(bytes12 _id, uint _checkpointIndex);
    ...
    function offer(bytes12 _id, address payable assigneeAddress, string memory _instructions) projectExists(_id) onlyClient(_id) payable public returns(bool) {
        ...
        projects[_id].instructions = _instructions;
    ...
    //mark checkpoint completed and request for transfer reward
    function requestReward(bytes12 _id, uint index) projectExists(_id) onlyOfferedAssignee(_id) isAssigned(_id) proposalDoesNotExist(_id) public returns(bool) {
        require(index < projects[_id].checkpointsCompleted.length, "Checkpoint index out of bounds");
        require(!projects[_id].checkpointsCompleted[index], "Checkpoint already completed");
        
        emit rewardRequested(_id, index);
    ...
    function getOfferedAssignee(bytes12 _id) view public projectExists(_id) returns(address) {
        return (
            projects[_id].offeredAssignee
        );
    }
