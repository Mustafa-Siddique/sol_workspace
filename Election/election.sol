...
contract election{
    ...
    constructor(address _owner, string memory _title, string memory _description, uint256 time){
        owner = _owner;
        electionName = _title;
    ...
    function addVoter(string memory _name, string memory _ppsn, uint128 _dob, string memory _address, string memory photo, string memory email) public {
        uint8 _id = numVoters;
        voters[_id] = Voter(_name, _ppsn, _dob, _address, photo, email, msg.sender, false, false);
        numVoters++;
    }
    ...
    function getVoterDetails(uint8 id) public view returns(Voter memory){
        require(msg.sender == owner || msg.sender == voters[id].wallet, "You don't have authority to access this information.");
        return voters[id];
    }

    function verifyVoter(uint8 id) public returns(string memory){
        require(msg.sender == voters[id].wallet, "You can't verify this voter.");
        voters[id].verified = true;
        return ("You have been verified successfully!");
    }

    function vote(uint8 _partyId, uint8 _voterId) public returns(string memory){
        require(msg.sender == voters[_voterId].wallet, "You can't vote on others behalf.");
