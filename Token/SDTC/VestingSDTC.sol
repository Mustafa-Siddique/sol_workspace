...
contract SDTCVesting{
    ...
    uint256 public teamAndInvestorsVestingEndTime = teamAndInvestorsVestingStartTime.add(teamAndInvestorsVestingPeriod);
    uint256 public teamAndAdvisorsVestingEndTime = teamAndAdvisorsVestingStartTime.add(teamAndAdvisorsVestingPeriod);

    uint256 public teamAndInvestorsReleasedAmount;
    uint256 public teamAndAdvisorsReleasedAmount;

    uint256 public teamAndAdvisorsWeeklyReleaseAmount = teamAndAdvisorsAmount.div(26);
    ...
    function setTeamAndInvestors(address payable _teamAndInvestors) external onlyOwner{
        require(teamAndInvestors != address(0), "Team and Investors address not set");

        teamAndInvestors = _teamAndInvestors;
        emit WalletChanged(msg.sender, _teamAndInvestors);
    }

    function setTeamAndAdvisors(address payable _teamAndAdvisors) external onlyOwner{
        require(teamAndAdvisors != address(0), "Team and Advisors address not set");
    ...
    function lockTeamAndInvestorsTokens() external onlyOwner{
        require(teamAndInvestors != address(0), "Team and Investors address not set");
        require(token.balanceOf(msg.sender) >= teamAndInvestorsAmount, "Insufficient balance");
        require(token.allowance(msg.sender, address(this)) >= teamAndInvestorsAmount, "Insufficient allowance");
    ...
    function lockTeamAndAdvisorsTokens() external onlyOwner{
        require(teamAndAdvisors != address(0), "Team and Advisors address not set");
        require(token.balanceOf(msg.sender) >= teamAndAdvisorsAmount, "Insufficient balance");
        require(token.allowance(msg.sender, address(this)) >= teamAndAdvisorsAmount, "Insufficient allowance");
    ...
    function releaseTeamAndInvestorsTokens() external onlyOwner{
        require(teamAndInvestors != address(0), "Team and Investors address not set");
        require(block.timestamp > teamAndInvestorsVestingEndTime, "Vesting period not started yet");
        require(teamAndInvestorsReleasedAmount < teamAndInvestorsAmount, "All tokens released");

        uint256 releasableAmount = teamAndInvestorsAmount;
        require(releasableAmount > 0, "No tokens to release");

        token.transfer(teamAndInvestors, releasableAmount);
        teamAndInvestorsReleasedAmount = teamAndInvestorsReleasedAmount.add(releasableAmount);
        emit Release(teamAndInvestors, releasableAmount);
    }

    function releaseTeamAndAdvisorsTokens() external onlyOwner{
        require(teamAndAdvisors != address(0), "Team and Advisors address not set");
        require(block.timestamp > teamAndAdvisorsVestingStartTime, "Vesting period not started yet");
        require(teamAndAdvisorsReleasedAmount < teamAndAdvisorsAmount, "All tokens released");

        uint256 weeksPassed = (block.timestamp.sub(teamAndAdvisorsVestingStartTime)).div(7 days);
        uint256 releasableAmount = weeksPassed.mul(teamAndAdvisorsWeeklyReleaseAmount);
        ...
        teamAndAdvisorsReleasedAmount = teamAndAdvisorsReleasedAmount.add(releasableAmount);
